...
**Nachjustierung Timing, nur Voice (29. Juli 2026, nach Nutzertest):** Getrennt vom Tonproblem zeigte sich ein zweites: `TranscriptWriter` reagierte im Voice-Modus sofort, sobald ein akutes Wort in einem gerade finalisierten STT-Segment auftauchte — also nach der normalen VAD-Pause von 2 Sekunden. Für den Coachee fühlte sich das wie Dazwischenreden an, wenn nach dem akuten Wort nur kurz Luft geholt wurde, nicht der Gedanke zu Ende war. Behoben durch eine zusätzliche Wartezeit (`CRISIS_GRACE_SECS = 4.0`, `voice-agent/bot.py`): Spricht der Coachee innerhalb dieser Zeit weiter, wird der neue Text angehängt und die Wartezeit neu gestartet, statt zu unterbrechen — erst nach echter Ruhe kommt die Antwort. Der Auslöser bleibt hart: einmal erkannt, wird immer reagiert, auch wenn im Anschluss abgewiegelt wird. Textmodus ist von diesem Problem nicht betroffen (Senden ist dort eine explizite, bewusste Nutzerhandlung, kein durch Stille interpretiertes Ende).
